...
if ($pdo) {
    ...
    // Select all users from the database and add them to the list from which you can choose which user is writing the post
    ...
    // Run if/else when the user presses "Skicka"
    if (isset($_POST['submit'])) {
        $user = $_POST['author']; // Store the author's name in the $user variable
        $headline = $_POST['title']; // Store the title of the post in the $headline variable
        $headline = trim($headline);

        $slug = slugify($headline); // Apply the slugify function on the $headline variable and store the result into $slug

        $text = $_POST['message']; // Store the content of the post in the $text variable
